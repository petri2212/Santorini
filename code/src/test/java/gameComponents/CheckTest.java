...
class CheckTest {
    ...
    void setUp() {
        board = new Board(5, 5);
        playerRed = new Player("A", PlayerColor.RED);
        redWorker1 = playerRed.getWorkers()[0];
        redWorker2 = playerRed.getWorkers()[1];
        playerBlue = new Player("B", PlayerColor.BLUE);
    ...
    void testPossibleMovement() {
        ...
        for (int dx = -1; dx <=1; dx++) {
            for (int dy = -1; dy <=1; dy++) {
                ...
                if (Check.isInsideBoard(board, newX, newY)) {
                    ...
                    cell.setWorker(new Worker(PlayerColor.BLUE)); // Place a worker to block
                }
            }
        }
        ...
        for (int dx = -1; dx <=1; dx++) {
            for (int dy = -1; dy <=1; dy++) {
                ...
                if (Check.isInsideBoard(board, newX, newY)) {
                    Cell cell = board.cellAt(newX, newY);
                    cell.setWorker(new Worker(PlayerColor.RED)); // Place a worker to block
                }
            }
        }
